...
try:
    ...
    for sym in STOCKS:
        ...
        if not df.empty and len(df) > 50:
            ...
            # --- إضافة مؤشر الاستباق (Squeeze & Volume Accumulation) ---
            bbands = ta.bbands(df['Close'], length=20, std=2)
            bb_width = (bbands['BBU_20_2.0'] - bbands['BBL_20_2.0']) / bbands['BBM_20_2.0']
            is_squeeze = bb_width.iloc[-1] < bb_width.rolling(20).mean().iloc[-1] # ضيق القناة
            ...
            # --- منطق الإشارة المطور للاستباق ---
            icon, status, row_class, quality = "⚪", "هدوء", "row-calm", "-"
            
            # حالة الاستباق: إذا كان هناك تجميع فوليوم والقناة ضيقة قبل تقاطع MACD
            pre_entry = ""
            if is_squeeze and v_ratio > 1.1:
                pre_entry = "⚡" # علامة الاستباق
            ...
            # دمج علامة الاستباق مع الأيقونة
            final_icon = f"{pre_entry}{icon}" if pre_entry else icon

            results.append({
                "⚡": final_icon, "S": sym, "CH": f"{change_pct:+.2f}%", 
    ...
    with col_leg:
        st.markdown("""
            <div class="legend-box">
                ...
                <div class="leg-item">⚡ <b>إشارة استباقية:</b> تجميع سيولة</div>
